'''
EfficientNet implementation with FiLM layers added. Copied/adapted from: https://github.com/pytorch/vision/blob/main/torchvision/models/efficientnet.py
'''
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        film_context_size: Optional[int] = None,
        ...
        Args:
            ...
            num_classes (int): Number of classes. If None, the model will not include the classification head
            film_context_size (Optional[int]): The FiLM conditioning context size. FiLM conditioning is only applied if a value is provided
            norm_layer (Optional[Callable[..., nn.Module]]): Module specifying the normalization layer to use
            last_channel (int): The number of channels on the penultimate layer, before the classification head
        """
        super().__init__()
        ...
        self.use_film = film_context_size is not None
        self.use_head = num_classes is not None
        ...
        for cnf in inverted_residual_setting:
            ...
            for _ in range(cnf.num_layers):
                ...
                if self.use_film:
                    stage.append(FilmConditioning(block_cnf.out_channels, context_size=film_context_size))
        ...
        self.lastconv_output_channels = last_channel if last_channel is not None else 4 * lastconv_input_channels
        layers.append(
            Conv2dNormActivation(
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                if self.use_head and m.weight.data.shape[0] == num_classes:
                    init_range = 1.0 / math.sqrt(m.out_features)
                    nn.init.uniform_(m.weight, -init_range, init_range)
                    nn.init.zeros_(m.bias)

    def _forward_impl(self, x: Tensor, context: Tensor) -> Tensor:
        if self.use_film and context is None:
            raise ValueError("Context tensor is required when using FiLM")

        for layer in self.features:
            if isinstance(layer, nn.ModuleList):
                for block in layer:
                    if isinstance(block, FilmConditioning):
                        x = block(x, context)
                    else:
                        x = block(x)
            else:
...
def _efficientnet(
    ...
    **kwargs: Any,
    ...
    if weights is not None and kwargs.get('num_classes') is not None:
        _ovewrite_named_param(kwargs, "num_classes", len(weights.meta["categories"]))

    model = EfficientNet(inverted_residual_setting, dropout, last_channel=last_channel, **kwargs)

    if weights is not None:
        pretrained_state_dict = weights.get_state_dict(progress=progress, check_hash=True)

        if kwargs.get('num_classes') is None:
            pretrained_state_dict.pop("classifier.1.weight")                                                           # Remove classification head weights
            pretrained_state_dict.pop("classifier.1.bias")

        if kwargs.get('film_context_size') is not None:
            state_dict = model.state_dict()

            for key in pretrained_state_dict.keys():
                if key.startswith("features") and "block" in key:
                    key_list = key.split(".")
                    idx_2 = str(int(key.split(".")[2]) * 2)                                                            # Fix the index to match the model with the FiLM conditioning layers 
                    new_key = ".".join(key_list[:2] + [idx_2] + key_list[3:])
                    state_dict[new_key] = pretrained_state_dict[key]
                else:
                    state_dict[key] = pretrained_state_dict[key]

        model.load_state_dict(state_dict if kwargs.get('film_context_size') is not None else pretrained_state_dict)
